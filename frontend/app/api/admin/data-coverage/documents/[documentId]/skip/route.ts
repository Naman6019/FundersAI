...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    const res = await fetch(backendUrl(`/api/admin/mf-documents/${encodeURIComponent(documentId)}/skip`), {
      method: 'POST',
      cache: 'no-store',
      headers: {
      ...
      body: JSON.stringify({ reviewer_notes: 'Skipped as irrelevant from admin data coverage.' }),
    });
