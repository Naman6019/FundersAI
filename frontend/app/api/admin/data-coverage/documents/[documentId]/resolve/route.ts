...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    const res = await fetch(backendUrl(`/api/admin/mf-documents/${encodeURIComponent(documentId)}/resolve`), {
      method: 'POST',
      cache: 'no-store',
      headers: {
      ...
      body: JSON.stringify({ reviewer_notes: 'Resolved from admin data coverage.' }),
    });
