-- Make MF promotion reversible.
--
-- `promote_mf_factsheet_candidate` already captured before/after snapshots into
-- `mf_promotion_runs`, but `promote_mf_holdings_document_v2` omitted
-- `before_snapshot` entirely while deleting live `mutual_fund_holdings` /
-- `mutual_fund_sectors` rows -- so a bad portfolio promotion destroyed the prior
-- rows with no record of them. This migration:
--   1. adds revert bookkeeping to `mf_promotion_runs`,
--   2. replaces the portfolio promotion function so it records the runtime rows it
--      is about to destroy *and* the full set of scheme codes it touches, and
--   3. adds `revert_mf_promotion_run`, which restores either promotion kind from
--      its captured before-state inside one transaction.
--
-- Applying this migration does not promote or revert any data. Runs applied
-- before it carry an empty `before_snapshot` and are refused by the revert
-- function rather than restored from nothing.

alter table public.mf_promotion_runs
  add column if not exists reverted_at timestamptz,
  add column if not exists reverted_by text,
  add column if not exists revert_of_run_id uuid references public.mf_promotion_runs(id);

alter table public.mf_promotion_runs
  drop constraint if exists mf_promotion_runs_status_check;

alter table public.mf_promotion_runs
  add constraint mf_promotion_runs_status_check
  check (status in ('dry_run', 'applied', 'rejected', 'failed', 'reverted'));

create index if not exists mf_promotion_runs_revertable_idx
  on public.mf_promotion_runs (amc_code, created_at desc)
...
declare
  ...
  holdings_before jsonb := '[]'::jsonb;
  holdings_before_codes jsonb := '[]'::jsonb;
  sectors_before jsonb := '[]'::jsonb;
  sectors_before_codes jsonb := '[]'::jsonb;
begin
  ...
  if 'holdings' = any(requested_scopes) then
    select coalesce(jsonb_agg(to_jsonb(runtime)), '[]'::jsonb)
    into holdings_before
    from public.mutual_fund_holdings runtime
    ...
    select coalesce(jsonb_agg(distinct affected.scheme_code), '[]'::jsonb)
    into holdings_before_codes
    from (
        select distinct h.mapped_scheme_code::integer
        from public.mf_scheme_holdings h
        join public.mutual_fund_family_mapping m
  ...
  if 'sectors' = any(requested_scopes) and staged_sector_count > 0 then
    select coalesce(jsonb_agg(to_jsonb(runtime)), '[]'::jsonb)
    into sectors_before
    from public.mutual_fund_sectors runtime
    where runtime.source = 'amc_disclosure'
      and runtime.scheme_code in (
  ...
  insert into public.mf_promotion_runs (
    ...
    jsonb_build_object(
      ...
      'holdings_scheme_codes', holdings_before_codes,
      'sectors', sectors_before,
      'sectors_scheme_codes', sectors_before_codes
    ),
    jsonb_build_object(
...
-- Restore a single applied promotion run from its captured before-state.
create or replace function public.revert_mf_promotion_run(
  p_run_id uuid,
  p_requested_by text
) returns jsonb
language plpgsql
...
as $$
declare
  run public.mf_promotion_runs%rowtype;
  before_core public.mutual_fund_core_snapshot%rowtype;
  report_month date;
  holdings_codes integer[] := '{}'::integer[];
  sectors_codes text[] := '{}'::text[];
  removed_holdings integer := 0;
  restored_holdings integer := 0;
  removed_sectors integer := 0;
  restored_sectors integer := 0;
  restored_core integer := 0;
  revert_kind text;
  audit_id uuid;
begin
  select * into run
  ...
  if run.id is null then
    raise exception 'promotion_run_not_found';
  end if;
  if run.status <> 'applied' then
    raise exception 'promotion_run_not_applied';
  end if;
  if run.reverted_at is not null then
    raise exception 'promotion_run_already_reverted';
  end if;
  if run.before_snapshot is null
     or jsonb_typeof(run.before_snapshot) <> 'object'
     or run.before_snapshot = '{}'::jsonb then
    raise exception 'promotion_run_before_snapshot_unavailable';
  end if;

  if run.candidate_id is not null then
    revert_kind := 'core';

    if run.before_snapshot->>'scheme_code' is null then
      raise exception 'promotion_run_before_snapshot_unavailable';
    end if;

    before_core := jsonb_populate_record(
      null::public.mutual_fund_core_snapshot,
      run.before_snapshot
    );

    update public.mutual_fund_core_snapshot s
    set
      risk_level = case
        when 'risk' = any(run.scopes) then before_core.risk_level
        else s.risk_level
      end,
      expense_ratio = case
        when 'ter_aum' = any(run.scopes) then before_core.expense_ratio
        else s.expense_ratio
      end,
      aum = case
        when 'ter_aum' = any(run.scopes) then before_core.aum
        else s.aum
      end,
      benchmark = case
        when 'benchmark' = any(run.scopes) then before_core.benchmark
        else s.benchmark
      end,
      fund_manager = case
        when 'manager' = any(run.scopes) then before_core.fund_manager
        else s.fund_manager
      end,
      provider_payload = before_core.provider_payload,
      last_updated = now()
    where s.scheme_code = before_core.scheme_code;
    get diagnostics restored_core = row_count;

    if restored_core = 0 then
      raise exception 'promotion_run_target_scheme_missing';
    end if;

    -- Release the reverted scopes so the candidate can be re-promoted later.
    update public.mf_factsheet_candidates c
    set
      promoted_scopes = (
        select coalesce(array_agg(distinct scope), '{}')
        from unnest(coalesce(c.promoted_scopes, '{}')) scope
        where scope <> all(run.scopes)
      ),
      ...
      updated_at = now()
    where c.id = run.candidate_id;

  else
    revert_kind := 'portfolio';

    if not coalesce((run.before_snapshot->>'revertable')::boolean, false) then
      raise exception 'promotion_run_before_snapshot_unavailable';
    end if;

    report_month := (run.before_snapshot->>'report_month')::date;
    if report_month is null then
      raise exception 'promotion_run_before_snapshot_unavailable';
    end if;

    if 'holdings' = any(run.scopes) then
      select coalesce(array_agg(code::integer), '{}'::integer[])
      into holdings_codes
      from jsonb_array_elements_text(
        coalesce(run.before_snapshot->'holdings_scheme_codes', '[]'::jsonb)
      ) as codes(code);

      delete from public.mutual_fund_holdings
      where source = 'amc_disclosure'
        and as_of_date = report_month
        and scheme_code = any(holdings_codes);
      get diagnostics removed_holdings = row_count;

      insert into public.mutual_fund_holdings
      select *
      from jsonb_populate_recordset(
        null::public.mutual_fund_holdings,
        coalesce(run.before_snapshot->'holdings', '[]'::jsonb)
      );
      get diagnostics restored_holdings = row_count;
    end if;

    if 'sectors' = any(run.scopes) then
      select coalesce(array_agg(code), '{}'::text[])
      into sectors_codes
      from jsonb_array_elements_text(
        coalesce(run.before_snapshot->'sectors_scheme_codes', '[]'::jsonb)
      ) as codes(code);

      delete from public.mutual_fund_sectors
      where source = 'amc_disclosure'
        and scheme_code = any(sectors_codes);
      get diagnostics removed_sectors = row_count;

      insert into public.mutual_fund_sectors
      select *
      from jsonb_populate_recordset(
        null::public.mutual_fund_sectors,
        coalesce(run.before_snapshot->'sectors', '[]'::jsonb)
      );
      get diagnostics restored_sectors = row_count;
    end if;
  end if;

  update public.mf_promotion_runs
  set
    reverted_at = now(),
    reverted_by = p_requested_by
  where id = run.id;
  ...
  insert into public.mf_promotion_runs (
    ...
    jsonb_build_object(
      ...
      'reverted_run_completed_at', run.completed_at
    ),
    run.after_snapshot,
    jsonb_build_object(
      'restored_core_rows', restored_core,
      'removed_holdings_rows', removed_holdings,
      'restored_holdings_rows', restored_holdings,
      'removed_sectors_rows', removed_sectors,
      'restored_sectors_rows', restored_sectors
    ),
    run.id,
    now()
  )
  ...
  return jsonb_build_object(
    ...
    'revert_kind', revert_kind,
    'reverted_run_id', run.id,
    'revert_run_id', audit_id,
    'amc_code', run.amc_code,
    'scopes', run.scopes,
    'restored_core_rows', restored_core,
    'removed_holdings_rows', removed_holdings,
    'restored_holdings_rows', restored_holdings,
...
revoke all on function public.revert_mf_promotion_run(uuid, text)
  from public, anon, authenticated;
grant execute on function public.revert_mf_promotion_run(uuid, text)
  to service_role;
