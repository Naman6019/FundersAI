...
from app.services import cache_policy, indianapi_service
...
def get_mf_repository() -> MutualFundRepository:
    return _current_mf_repository.get()
...
def fetch_quant_data(ticker: str, period: str = "1mo") -> dict:
    ...
    def get_local_quant_snapshot(symbol: str) -> dict | None:
        ...
        try:
            snapshot_row = None
            snapshot_res = get_mf_repository().table('nifty_stocks').select('*').eq('symbol', symbol).limit(1).execute()
            if snapshot_res.data:
                snapshot_row = snapshot_res.data[0]

            history_res = get_mf_repository().table('stock_prices_daily').select('close, date').eq('symbol', symbol).order('date', desc=True).limit(2).execute()
...
async def analyze_news_sentiment(news_items: list, usage_collector: list[dict[str, Any]] | None = None) -> list:
    langfuse.update_current_span(input=[n["title"] for n in news_items])
...
def _fund_name_words(value: str) -> list[str]:
    return re.findall(r"[a-z0-9]+", str(value or "").lower())


def _has_mf_category_hint(words: list[str]) -> bool:
    joined = " ".join(words)
    return any(token in words for token in ("fund", "cap", "flexi", "mid", "large", "small", "nav", "amc")) or "flexi cap" in joined


def _looks_like_ppfas_alias(value: str) -> bool:
    words = _fund_name_words(value)
    if not words:
        return False
    word_set = set(words)
    ...
    if word_set.intersection({"parag", "parikh"}):
        return True
    if not _has_mf_category_hint(words):
        return False

    common_typos = {"paras", "parakh", "parik", "prag", "paraag", "pfas"}
    if word_set.intersection(common_typos):
        return True
...
def _looks_like_axis_alias(value: str) -> bool:
    words = _fund_name_words(value)
    if not words:
        return False
    word_set = set(words)
...
def _is_supported_mf_query_text(value: str) -> bool:
    ...
    if _looks_like_ppfas_alias(low):
        return True
    if _looks_like_axis_alias(low):
        return True
    return any(marker in low for markers in SUPPORTED_MF_AMC_MARKERS.values() for marker in markers)


def _normalize_compare_entity_name(entity: str) -> str:
    text = " ".join(str(entity or "").replace("-", " ").replace(",", " ").replace(".", " ").split())
...
def _extract_compare_followup_question(query: str) -> str | None:
    ...
    if not text:
        return None
    match = re.search(r"(?:[\.,;]\s*|\b(?:and|,)\s+)((?:why|how|what|which|explain|show|tell)\b.+)$", text, flags=re.IGNORECASE)
    if match:
...
def _strip_compare_followup_clause(text: str) -> str:
    return re.sub(
        r"(?:[\.,;]\s*|\b(?:and|,)\s+)(?:why|how|what|which|explain|show|tell)\b.+$",
        "",
        text,
        flags=re.IGNORECASE,
    ).strip()
...
def _compare_intent_has_specific_entities(intent: dict[str, Any] | None) -> bool:
    if not intent:
        return False
    entities = [str(entity or "").strip() for entity in intent.get("compare_entities") or []]
    if len(entities) < 2:
        return False
    generic_markers = ("all ", "top ", "best ", "category", "funds", "schemes")
    for entity in entities[:2]:
        low = entity.lower()
        if any(marker in low for marker in generic_markers) and not _is_supported_mf_query_text(entity):
            return False
        if not _is_supported_mf_query_text(entity):
            return False
    return True
...
def _deterministic_compare_intent(query: str, asset_type: str = "auto") -> dict[str, Any] | None:
    ...
    text = " ".join(str(query or "").strip().split())
    low = text.lower()
    has_compare_verb = "compare" in low
    has_vs_separator = re.search(r"\s+(?:vs\.?|versus)\s+", text, flags=re.IGNORECASE) is not None
    if not has_compare_verb and not has_vs_separator:
        return None
    if not any(token in low for token in ("fund", "flexi", "cap", "ppfas", "parag", "hdfc", "icici", "sbi", "axis", "axs")):
        return None

    cleaned = re.sub(r"^compare\s+", "", _strip_compare_followup_clause(text), flags=re.IGNORECASE)
    ...
    intent = {
        ...
        "sentiment_flag": False,
        "downside_focus": any(token in low for token in ("downside", "drawdown", "fall", "crash", "protection")),
...
def _normalize_portfolio_fund_name(name: str) -> str:
    ...
    if ("parag" in low or "ppfas" in low) and "flexi" in low:
        return "Parag Parikh Flexi Cap"
    if _looks_like_axis_alias(low) and "flexi" in low:
        return "Axis Flexi Cap"
    if _looks_like_axis_alias(low) and "large" in low and "cap" in low:
        return "Axis Large Cap"
    ...
    try:
        ...
        if best_match:
            if not best_match.get("aum") or not best_match.get("expense_ratio"):
                ...
                if code:
                    legacy_res = get_mf_repository().table("mutual_funds").select("aum,expense_ratio").eq("scheme_code", code).limit(1).execute()
                    if legacy_res.data:
...
def _build_category_compare_payload(category_key: str, scheme_codes: list[str]) -> dict[str, Any]:
    ...
    if category_key not in CATEGORY_SEARCH_CONFIG:
        raise BadRequestError("Unsupported category.")
    ...
    if len(clean_codes) < 2 or len(clean_codes) > 3:
        raise BadRequestError("Select 2 to 3 supported funds.")
    ...
    if missing:
        raise EntityNotFoundError(f"Fund data not found for: {', '.join(missing)}")
    unsupported = [str(row.get("scheme_name") or row.get("scheme_code")) for row in rows if not row.get("is_supported")]
    if unsupported:
        raise UnsupportedEntityError(f"Coming Soon: {', '.join(unsupported)}")
...
def _comparison_followup_answer_markdown(quant_data: Any, question: str | None) -> str:
    ...
    if any(token in low for token in ("return", "returns", "differ", "difference", "performance", "better", "long term", "long-term")):
    ...
    if any(token in low for token in ("risk", "safer", "steadier", "drawdown", "volatility", "sharpe", "downside", "protection")):
...
async def get_nifty_history_df(days: int = 1100):
    ...
    try:
        def _fetch_nifty():
            return get_mf_repository().table('stock_prices_daily').select('close, date').eq('symbol', 'NIFTY').order('date', desc=True).limit(days).execute()
...
async def compare_stocks_quant(symbols: str = Query(..., min_length=1)):
    try:
        ...
        raise DataUnavailableError("Stock quant comparison failed")
...
async def stock_quant_profile(symbol: str):
    try:
        ...
        raise DataUnavailableError("Stock profile failed")
...
async def stock_quant_financials(symbol: str):
    try:
        ...
        raise DataUnavailableError("Stock financials failed")
...
async def stock_quant_price_history(symbol: str, days: int = Query(365, ge=1, le=5000)):
    try:
        ...
        raise DataUnavailableError("Stock price history failed")
...
async def chat_endpoint(
    ...
    resolution_payload: list[dict[str, Any]] = []
    ...
    def _attach_runtime_meta(response: dict[str, Any]) -> dict[str, Any]:
        ...
        response["coverage_status"] = response.get("coverage_status") or coverage_status
        response["model_status"] = response.get("model_status") or model_status
        response["status_flag"] = response.get("status_flag") if "status_flag" in response else status_flag
        if resolution_payload and "resolution" not in response:
            response["resolution"] = resolution_payload
        if data_status and "data_status" not in response:
            response["data_status"] = data_status
        logger.info(
            "chat trace_id=%s intent=%s coverage=%s model=%s status_flag=%s elapsed_ms=%s",
            trace_id,
            response.get("debug_intent", {}).get("intent") if isinstance(response.get("debug_intent"), dict) else None,
            response.get("coverage_status"),
            response.get("model_status"),
    ...
    if dashboard_intent and dashboard_intent.get("intent") == "category_search" and deterministic_compare and not _compare_intent_has_specific_entities(deterministic_compare):
    ...
    if is_mf_context and intent in ["compare", "quant", "both"]:
        ...
        if intent == "compare":
            ...
            if entities:
                compare_resolutions = resolver.resolve_many([str(ent) for ent in entities], asset_type="mutual_fund")
                resolution_payload = [item.client_payload() for item in compare_resolutions]
                normalized_entities = [
                    item.resolved_name if item.is_high_confidence and item.resolved_name else _normalize_compare_entity_name(str(ent))
                    for ent, item in zip(entities, compare_resolutions)
                ]
                intent_info["compare_entities"] = normalized_entities
                coverage_status = "partial"
                for item in compare_resolutions:
                    if item.coverage_status == "unsupported":
                        is_unsupported_mf = True
                        break
            else:
                is_unsupported_mf = True
        elif intent in ["quant", "both"]:
            single_resolution = resolver.resolve(str(ticker or req.query), asset_type="mutual_fund")
            resolution_payload = [single_resolution.client_payload()]
            coverage_status = single_resolution.coverage_status
            if single_resolution.coverage_status == "unsupported":
                is_unsupported_mf = True
...
- Asking about NAV trends, expense ratios, or Sharpe/Alpha comparisons across these supported AMCs."""
    ...
    if intent == "screen":
        ...
        if len(entities) == 1:
            ...
            if asset_type == "stock":
                ...
                quant_data = compare_payload["quant_data"]
                intent_info["compare_entities"] = compare_payload.get("entities", entities)
    ...
    if (intent == "news" or (intent == "compare" and req.comparison_view_mode != "canvas")) and not news_data:
        news_items = fetch_news(req.query, ticker)
        if sentiment:
            news_items = await analyze_news_sentiment(news_items, usage_collector=usage_collector)
        news_data = news_items
...
class ChatService:
    def __init__(self, repository: MutualFundRepository | None = None):
        self.repository = repository or _default_mf_repository
...
class FundCategoryService:
    ...
    def list_category(self, category: str) -> dict[str, Any]:
        token = _current_mf_repository.set(self.repository)
        try:
            category_key = _category_key_from_value(category)
            if not category_key:
...
class MutualFundDetailService:
    ...
    async def get_details(self, scheme_code: int, background_tasks: BackgroundTasks) -> dict[str, Any]:
        token = _current_mf_repository.set(self.repository)
        try:
