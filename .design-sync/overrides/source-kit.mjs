// forked from design-sync lib/source-kit.mjs — adds cfg.excludeSrcFiles
// filtering on srcFiles (see step 1) so files that import next/image
// (framework-runtime-only; crashes the whole standalone esbuild bundle at
// module-load time with "process is not defined") never reach entry
// synthesis or component discovery. See .design-sync/NOTES.md.
//
...
export async function resolvePackage(ctx) {
  ...
  // forked: cfg.excludeSrcFiles — package-relative substrings/paths of src
  // files to drop from entry synthesis AND discovery entirely (not just the
  // components list — componentSrcMap:null can't stop a file's top-level
  // imports from being bundled). Use for files whose imports can't run
  // outside their framework's own runtime (e.g. next/image).
  const EXCLUDE_SRC = (cfg.excludeSrcFiles ?? []).map((s) => slash(s));
  if (EXCLUDE_SRC.length) {
    const before = srcFiles.length;
    srcFiles = srcFiles.filter((p) => !EXCLUDE_SRC.some((ex) => slash(p).includes(ex)));
    const dropped = before - srcFiles.length;
    if (dropped) console.error(`  [EXCLUDE_SRC_FILES] dropped ${dropped} file(s) matching cfg.excludeSrcFiles from entry synthesis + discovery`);
  }
  ...
  if (!entry) {
    ...
    // forked: `export * from` never re-exports a file's `default` export (ES
    // module semantics) — components authored as `export default function
    // Name()` were silently absent from window.<GLOBAL> even though
    // deriveComponentsFromSrc (below) correctly discovered their recovered
    // name for the catalog. Add an explicit named re-export per file with a
    // detected default export, alongside the star re-export.
