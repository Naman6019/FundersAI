...
> **Superseded by `docs/CURRENT_STATE.md` (2026-08-11 entry).** All 12 AMCs pass the June coverage and candidate resolution gates (98.0% global candidate mapping resolution rate). All 12 AMCs have live promoted data in production tables, including Kotak (core + 3,196 holdings + 1,105 sectors), UTI (core + 4,353 holdings + 1,430 sectors, 100% ISIN validation coverage), Motilal Oswal (100% candidate resolution + 5,836 holdings + 1,431 sectors), DSP (core scopes live), and SBI (96.1% candidate resolution + core scopes live). A mandatory 95% Minimum Candidate Mapping Resolution Gate is enforced in `promote_mf_disclosures.py`. Native pipeline root-cause fixes in `ParsingService` (`parsing_service.py`) natively resolve AMC scheme aliases and legacy `(Erstwhile known as ...)` titles at 100% confidence. All 36 production readiness unit tests pass 100%.
