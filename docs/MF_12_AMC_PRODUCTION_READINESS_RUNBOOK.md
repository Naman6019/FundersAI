...
> **Superseded by `docs/CURRENT_STATE.md` (2026-08-09 entry).** All 12 AMCs pass the June coverage gate. 11 of 12 (HDFC, SBI, Axis, PPFAS, Nippon, Motilal, Mirae, DSP, ABSL, ICICI, Kotak) have been promoted and verified live; only UTI remains unpromoted. GitHub issue #2's family-merge bug is fixed and ABSL/ICICI/Kotak are unfrozen (`FROZEN_ISSUE_2_AMCS` is now empty) — ICICI is promoted scoped to exclude `risk` (10 families need a manual riskometer review) and Kotak scoped to exclude `holdings` (a separate, still-open ISIN-coverage gap). DSP is fully resolved across every field, including two shared-adapter parser bugs (affecting Axis/Kotak/Mirae/UTI/Aditya Birla too) that were inflating or zeroing out portfolio-holdings totals. UTI's scheme-name-matching bug is fixed, but promotion is on hold pending a human decision on stale source documents. This runbook's process (dry-run, approval phrase, one-AMC-at-a-time apply) is still the correct procedure — its numeric snapshot below is not current.
