...
- Twelve-AMC production readiness is implemented and production staging is active, but full runtime activation is not complete. See `MF_12_AMC_PRODUCTION_READINESS_RUNBOOK.md` for the execution procedure:
  ...
  - the latest read-only June promotion-eligible report passes every 80% field and mapping gate for 11 of 12 AMCs. SBI, ICICI, Axis, PPFAS, Nippon, Motilal, Mirae, UTI, DSP, Kotak, and Aditya Birla pass. After the per-scheme XLSX parse, HDFC remains below the portfolio gates at 34.74% holdings and 37.65% sectors; its core fields and both mapping gates pass;
  ...
  - staging-only deterministic parse `30428575262` processed the 107 XLSX files plus two pending HDFC factsheets with LLM and runtime writes disabled. It completed with zero runtime errors, but 107 documents remained actionable because GitHub installed unpinned pandas `3.0.5`: merged summary cells became `nan` instrument rows carrying 100% values, inflating correct portfolio totals to 300-450%. The repository fix now treats non-finite instrument cells as empty and pins pandas `2.3.3`; the exact acquired workbook returns 94 holdings, 100.0% total weight, and no warning under both pandas `2.3.3` and `3.0.5`. This fix is locally verified but not yet deployed or reparsed;
  ...
  - remaining production gates are: deploy the HDFC pandas/null fix, reparse the June HDFC documents from staging-only statuses, rerun promotion-eligible coverage, run current independent-scope dry-runs, apply only passing scopes, verify runtime coverage and selected live APIs, verify the exact HDFC `118989` versus Nippon `118668` chat/canvas acceptance case, verify Data & Trust, and only then enable each new AMC for users;
