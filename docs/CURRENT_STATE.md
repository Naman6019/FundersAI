...
- Twelve-AMC production readiness is implemented and production staging is active, but full runtime activation is not complete. See `MF_12_AMC_PRODUCTION_READINESS_RUNBOOK.md` for the execution procedure:
  ...
  - the corrected June promotion-eligible report currently passes every 80% field and mapping gate for 9 of 12 AMCs: SBI, ICICI, Axis, PPFAS, Motilal, Mirae, UTI, DSP, and Aditya Birla. HDFC remains below the portfolio gates at 35.96% holdings and 39.74% sectors, Nippon at 74.68% and 76.47%, and Kotak at 73.97% and 73.24%. Those three remain blocked from portfolio promotion;
  ...
  - HDFC's reviewed June factsheet sources now use the official corrected `_1.pdf` main and Index Solutions consolidated files. Discovery locally ranks `_1` revisions ahead of the original filename while retaining the original as a probed fallback. These PDFs are valid factsheet sources, but they do not replace HDFC's per-scheme monthly XLSX files for complete holdings: the Index Solutions factsheet publishes top holdings plus aggregate residual exposure, so holdings and sector promotion remain blocked pending acquisition and staging of the official June XLSX set;
  ...
  - remaining production gates are: acquire and stage HDFC's complete June per-scheme portfolio XLSX set, deploy and run the bounded parser corrections, reparse Nippon and Kotak with their locally verified continuation/duplicate fixes, rerun independent-scope dry-runs, apply only passing scopes, verify runtime coverage and selected live APIs, verify the exact HDFC `118989` versus Nippon `118668` chat/canvas acceptance case, verify Data & Trust, and only then enable each new AMC for users;
  - latest tracked backend verification is `583 passed, 6 skipped`; the broad `backend/tests` discovery command remains obstructed by a pre-existing inaccessible `backend/tests/tmp` directory, so verification uses `--ignore=backend/tests/tmp`. The previously completed frontend verification remains 58 contract tests plus TypeScript, ESLint, and the Next.js production build.
- MF storage controls:
