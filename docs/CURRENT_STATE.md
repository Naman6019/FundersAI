...
- Comparison canvas delivery hardening:
  ...
  - local pipeline hardening separates the 24-hour request-serving cache TTL from metric-history readiness. Metric histories with at least 31 points remain ready for 14 days (`MF_METRIC_HISTORY_MAX_AGE_DAYS`), while bounded refresh still prioritizes missing, metric-stale, and oldest histories. A 100-scheme weekday rotation therefore covers all 871 schemes before readiness expires. The 2026-08-06 read-only production check found 870/871 ready histories and 867/871 ready histories with non-null alpha/beta (99.54%); deployment is pending.
  - reparsing and staging reconciliation now compare promoted candidates with their latest applied promotion snapshot. A changed scheme/family identity is preserved on the reviewed target, marked `needs_review`, and reported as `promoted_mapping_changed` instead of being silently retargeted. The read-only audit found nine existing historical drifts across SBI and Kotak; no candidate or disclosure document was mutated. Risk coverage remains 827/871 (94.95%), so the 95% scheduled gate and seven-run reliability count remain blocked pending reviewed data correction.
  - the 90% rollout gate uses the true supported intersection (ready history plus non-null alpha and beta divided by all supported mapped schemes), retries only Supabase HTTP 502/503/504/522/525 failures on repeat-safe operations, reconciles stale provider runs to `timed_out`, and scopes scheduled document automation to PPFAS, SBI, and Mirae. Nippon/UTI require exact approved document IDs; Axis/DSP/Motilal/HDFC are validation-only; ABSL/ICICI/Kotak remain frozen by GitHub issue #2. Focused pipeline verification passes 18 tests. The tracked backend suite passes 664 tests with 6 skipped; four unrelated rate-limit tests currently fail during import because the in-progress reports code requires the absent `langchain_openai` package.
