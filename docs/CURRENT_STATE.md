...
- Twelve-AMC production-readiness implementation is local and not deployed or activated:
  ...
  - `20260727_add_mf_extraction_staging_and_promotion.sql` adds staging/audit tables and service-role-only, scope-specific promotion functions. Production presence of both staging tables and the discovery-to-raw link column was verified on 2026-07-27. No promotion scope was applied;
  ...
  - production acquisition has stored and staged 93 exact-June Mirae portfolio workbooks in R2/Supabase without promotion. The production parser pass retained per-document diagnostics and staged the current holdings evidence;
  ...
  - the acquired Nippon file is labelled as June by its path but its dominant scheme content is dated May 2026. A deterministic content-month gate now sends this mismatch to `needs_review` before deterministic or LLM staging, so its missing risk and incomplete TER cannot be treated as June coverage;
  - Nippon factsheet discovery now uses the AMC's central disclosure page and treats the e-factsheet label as its publication month. The official July 2026 factsheet was content-confirmed as June 2026 and passed checksum and parser-smoke validation; the exact-June monthly portfolio also passes. Both remain unacquired;
  - report-month confirmation now prefers the dominant official document content over a publication-path month. This makes the official Aditya Birla Sun Life July 2026 factsheet a validated June 2026 discovery candidate because its fund data is dated June 30, while the document remains unacquired and unpromoted;
  - Aditya Birla portfolio discovery now reads the AMC's official resource API, rewrites its controlled CDN path to the equivalent AMC-host URL, and excludes fortnightly/half-yearly files. The exact June 30 portfolio ZIP passed official-host, checksum, archive, and parser-smoke validation but remains unacquired;
  - DSP's official portfolio-disclosure page now yields the exact June 30 month-end ZIP and filters fortnightly/performance workbooks. The ZIP passed bounded discovery validation but remains unacquired and unparsed;
  - UTI discovery now searches all six bounded API months instead of stopping at the newest non-empty response. A read-only June 2026 acceptance pass validated the English active and passive factsheets plus the consolidated portfolio ZIP as exact-month, official-source, checksum-confirmed, and parser-smoke-passed candidates; the two factsheets remain unacquired while the portfolio evidence is already staged;
  - HDFC exact-June combined factsheet/portfolio discovery now passes through its official dated bucket using a normal browser user agent; Axis active/passive combined factsheets pass through the official page API and bounded browser recovery. These documents remain unacquired;
  ...
  - the latest exact-June staging audit still has four full passes: SBI, ICICI, PPFAS, and Mirae. HDFC, Axis, Nippon, Motilal, and Aditya Birla have no June staged rows; UTI is portfolio-only with 97.47% reviewed mapping, DSP is core-only with 98.57% reviewed mapping, and Kotak is core-only with 95.73% reviewed mapping;
  - reviewed mapping coverage is SBI 92.47% core / 85.47% portfolio, ICICI 98.48% / 98.56%, PPFAS 100% / 100%, and Mirae 98.90% / 97.75%. Read-only source-document dry-runs pass for representative exact-June SBI, ICICI, PPFAS, Mirae, UTI, DSP, and Kotak candidates. Full-row, stable-order portfolio validation covers 5,735 SBI rows (4,840 promotable), 8,973 ICICI rows (8,634 promotable), 45 PPFAS rows (all promotable), 15 rows in a representative Mirae document (all promotable), and 4,355 UTI rows (4,252 promotable); unresolved rows remain staged for review and no promotion RPC was called;
  - current acquisition gaps remain explicit: HDFC, Axis, Nippon, and Aditya Birla have validated exact-June core/portfolio candidates but no June staged rows; UTI's exact-June active/passive factsheets are validated but unacquired; DSP's exact-June portfolio is discovered but not acquired; Kotak is factsheet-only in staging and its official portfolio listing remains WAF-protected; Motilal's official API still exposes May factsheet and April portfolio data. These AMCs are not production-ready;
  ...
  - current worktree verification: `514 passed, 6 skipped` backend tests (`backend/tests/tmp` excluded because the local directory is access-denied during collection), `58 passed` frontend contracts, TypeScript, ESLint, and the Next.js production build pass; all 20 workflow files parse as YAML. `uvx graphify update .` remains unavailable because the package resolver reports no installable `graphify` version.
- MF storage controls:
...
- Locally verified Discovery V2 enhancements (not deployed):
  ...
  - July 27 hosted run `30304766428` completed its official-source probes but failed while persisting the first `content_confirmed` observation because the deployed check constraint only accepts `confirmed` and `unconfirmed`; `20260728_allow_content_confirmed_discovery_month.sql` is prepared but not yet applied.
