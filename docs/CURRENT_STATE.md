...
- Twelve-AMC production readiness is implemented and production staging is active, but full runtime activation is not complete. See `MF_12_AMC_PRODUCTION_READINESS_RUNBOOK.md` for the execution procedure:
  ...
  - HDFC apply `30439503481`, SBI apply `30446246412`, Aditya Birla apply `30446266677`, and DSP apply `30446454487` succeeded. The read-only live runtime gate passes all fields for those four AMCs. Exact percentages are HDFC: 100% risk, 82.11% TER/AUM, 100% benchmark, 87.37% manager, 94.74% holdings, 94.25% sectors; SBI: 100%, 91.86%, 94.19%, 89.53%, 100%, 100%; Aditya Birla: 88.76%, 85.39%, 100%, 95.51%, 88.10%, 92.77%; DSP: 100%, 98.55%, 100%, 84.06%, 83.95%, 90.67%;
  ...
  - remaining production gates are: serialize and apply the eight green ICICI, Axis, PPFAS, Nippon, Motilal, Mirae, UTI, and Kotak batches; verify all-12 runtime coverage and selected live APIs; verify the exact HDFC `118989` versus Nippon `118668` chat/canvas acceptance case; verify Data & Trust; and only then enable each new AMC for users;
  - latest tracked backend verification is `593 passed, 6 skipped`; the HDFC pandas/null regression subset passes `43 passed, 4 skipped`, and the exact acquired XLSX also passes under the runner's pandas `3.0.5`. The broad `backend/tests` discovery command remains obstructed by a pre-existing inaccessible `backend/tests/tmp` directory, so verification uses `--ignore=backend/tests/tmp`. The frontend verification remains 58 contract tests plus TypeScript, ESLint, and the Next.js production build.
- MF storage controls:
