...
- Twelve-AMC production-readiness implementation is partially pushed, with a small local follow-up diff; it is not deployed or activated. See `MF_12_AMC_PRODUCTION_READINESS_RUNBOOK.md` for the current execution snapshot:
  - one capability registry now covers HDFC, SBI, ICICI, Axis, PPFAS, Nippon, Motilal, Mirae, UTI, DSP, Kotak, and Aditya Birla, including official hosts, document rules, browser-recovery permission, combined-factsheet behavior, and independent discovery/acquisition/parser/promotion/runtime flags;
  - Motilal uses its official bounded AEM document-search API. Its reviewed manifest now also pins the official July-published `Factsheet June 2026 Active` PDF because the bounded API previously returned stale May/April candidates. The PDF contains June 30 factsheet, portfolio, and sector data;
  - URL identity removes only allowlisted tracking/timestamp parameters, conditional ETag/Last-Modified requests can skip unchanged bodies, checksum reuse skips duplicate R2 upload/parsing, and cross-month checksum reuse is rejected for review;
  - Mirae, UTI, DSP, Kotak, and Aditya Birla have deterministic multi-scheme portfolio adapters; ZIP member, nested-archive, and worksheet failures remain attached to parse diagnostics and valid partial archives become `parsed_partial`;
  - factsheet fields and portfolio rows now preserve the raw AMC scheme spelling and reviewed scheme/family mapping in staging. Parser code no longer promotes directly into runtime comparison tables;
  - `20260727_add_mf_extraction_staging_and_promotion.sql` adds staging/audit tables and service-role-only, scope-specific promotion functions. Production presence of both staging tables and the discovery-to-raw link column was verified on 2026-07-27. `20260728_allow_content_confirmed_discovery_month.sql` is also applied and its live constraint accepts `confirmed`, `content_confirmed`, and `unconfirmed`. Acquisition and per-scope promotion are approved, but no promotion scope has yet been applied;
  - protected Kotak core dry-run `30346969848` succeeded, but apply run `30347133742` failed before its first mutation with `PGRST202`: production exposes only the legacy three-argument promotion RPC while the workflow requires the report-month-bound four-argument contract. `20260728_harden_mf_promotion_rpc_contract.sql` is prepared locally to add the bounded wrappers, revoke direct service-role access to the legacy signatures, and reject any portfolio document containing non-promotable rows. It requires migration approval and application before any promotion retry;
  - the first manual attempt to apply the RPC repair failed with PostgreSQL `42883` because the wrapper called the legacy function as `(uuid, text, text[])`; production exposes `(uuid, text[], text)`. No promotion ran. The migration is corrected to use the deployed argument order and must be reapplied before retrying promotion;
  - the corrected RPC migration was then applied successfully. Exact Kotak core dry-run `30351019376` passed, proving the report-month-bound contract is reachable. Apply run `30351222400` stopped on its first candidate with PostgreSQL `22023` because a legacy runtime `provider_payload` is a JSON scalar and the delegated function calls `jsonb_set` as if it were an object. That RPC transaction rolled back and the workflow did not advance to later candidates. `20260728_fix_mf_promotion_provider_payload.sql` is prepared to preserve any scalar under `legacy_provider_payload` and perform normalization plus promotion atomically; it requires approval and application before another promotion retry;
  - local parsing of the official June 2026 Axis factsheet now resolves 80 of 82 families for AUM, TER, benchmark, and scheme risk, and 81 of 82 for manager. Axis TER parsing now supports bare numeric cells, wrapped scheme names, and six-column retail-plan rows; its vector riskometer reader maps scheme needles separately from benchmark needles;
  - local parsing of the official July-published ABSL factsheet whose body reports June 2026 now resolves 94 of 96 families for AUM, 81 for TER, 95 for benchmark, 91 for manager, and 83 for scheme risk. The ABSL closing-AUM label and left-hand scheme riskometer are handled deterministically. These parser improvements are locally verified but are not deployed or reparsed in production yet;
  - UTI acquisition run `30348803792` reached the correct official July-published active and passive factsheets. The passive file reused source document `a2f18da8-116d-44f4-b1f9-0059407e0a56`; the active file stopped at `checksum_month_conflict` against `ea3b9305-2968-40f6-bc8c-18c24195d298`, which is stored under a different report month. No duplicate R2 object or raw-document row was created. A reviewed month correction is still required before the active file can be staged as June;
  - acquisition and promotion have separate manual workflows with exact confirmation phrases. Scheduled disclosure sync is parser-only, and promotion remains a dry run unless its explicit `apply` input and matching phrase are supplied. Both jobs reference the GitHub `Production-data` environment, which has `Naman6019` as the required reviewer and a custom deployment branch policy limited to `main`; self-review remains allowed and administrators may bypass the rule;
  - the HDFC `118989` versus Nippon `118668` snapshot comparison is a local acceptance contract, with unavailable holdings overlap represented as unavailable rather than `0%`;
  - production acquisition has stored exact-June HDFC, Nippon, UTI, DSP, Aditya Birla, and Mirae documents in R2/Supabase without promotion. Parser passes have staged evidence for all six with per-document diagnostics;
  - locally verified June factsheet extraction clears the 80% field gate for SBI, ICICI, PPFAS, DSP, Kotak, and Mirae on the available official files. SBI's active/passive files jointly cover 84.3% AUM, 88.2% TER, 88.2% benchmark, 82.4% manager, and 89.2% risk across 102 parsed records; ICICI exceeds 88% on every field across 68 records; Mirae exceeds 81% on every field across 91 records after non-scheme `Fund Facts` headings are excluded. Kotak exceeds 94% for AUM, TER, manager, and risk and 98% for benchmark; PPFAS reaches 100% for all five core fields; DSP reaches at least 80% for all five core fields;
  - one Nippon PDF labelled June by its path contains dominant May 2026 scheme data and remains `needs_review`. The official July-named PDF contains June 30 data and has now been acquired and staged as June after body validation. Local deterministic extraction from that file covers AUM 100%, TER 87.18%, benchmark 85.90%, manager 98.72%, and risk 98.72% across 78 records; production staging must be refreshed before this is a production pass;
  - report-month confirmation prefers official factsheet body dates over publication-path months. A local parser guard now defers factsheet filename-month checks to the body detector while retaining filename rejection for portfolio files. This follow-up is tested locally but not yet pushed;
  - the official `Empower Factsheet - June 2026` PDF was acquired and then correctly rejected for the June reporting month because its body contains dominant May 2026 data. The official July-published ABSL factsheet body is confirmed as June 2026 and is restored in the reviewed manifest. Its existing R2 row is currently classified as July, so an audited, content-confirmed report-month alias is required before parsing. Local extraction finds 96 records with 84.38% TER, 98.96% benchmark, and 94.79% manager coverage; AUM and risk remain below the production gate. The exact June 30 portfolio ZIP remains staged with 91.30% mapping, 100% holdings coverage, and 97.33% sector coverage;
  - DSP's exact June factsheet and month-end portfolio ZIP are acquired and staged. DSP now passes all mapping and field gates, with partial-document diagnostics retained;
  - UTI's June-publication factsheets contain May data. The exact June 30 data is in the official July active and passive Fund Watch PDFs. Those URLs are now in the reviewed manifest and local deterministic extraction across both files covers AUM 93.83%, TER 92.59%, benchmark 86.42%, manager 95.06%, and risk 83.95% across 81 unique scheme keys. Production acquisition and staging must be refreshed before UTI passes;
  - HDFC's main and Index Solutions June factsheets are acquired and staged and now pass the field and mapping gates. Axis's reviewed June combined factsheet is acquired and staged, but its current parser output still has zero TER and risk coverage, so Axis remains blocked;
  - the read-only production staging report uses stable ID-ordered paging and distinct current-month observed families rather than every historical snapshot family. SBI, ICICI, PPFAS, and Mirae pass the 80% field and reviewed-mapping gate in staging. Sector coverage separates genuinely non-applicable fund-of-funds, commodity ETFs, liquid ETFs, and duration funds from missing sector data; Mirae has 63 sector-applicable mapped families with sector data and 24 explicitly non-applicable families;
  - the latest exact-June staging audit has eight full passes: HDFC, SBI, ICICI, PPFAS, Nippon, Mirae, DSP, and Kotak. Axis, UTI, Aditya Birla, and Motilal remain blocked;
  - reviewed mapping coverage is HDFC 97.94% core / 98.89% portfolio, SBI 92.47% / 85.47%, ICICI 98.48% / 98.56%, PPFAS 100% / 100%, Nippon 98.72% / 100%, Mirae 98.90% / 97.75%, DSP 98.57% / 87.21%, Kotak 95.73% / 96.05%, Axis 96.34% / 90.70%, UTI 90.41% / 97.47%, Motilal 100% / 100%, and Aditya Birla 0% / 91.30%. Core promotion dry-runs for the first seven passing AMCs succeeded; no apply promotion has run;
  - exact official June sources are locally verified for Motilal and Kotak. Motilal and Kotak combined-factsheet portfolio extraction avoids the slow generic PDF table scan. Motilal yields 20 portfolio records, 19 in the valid allocation band; Kotak yields 76 records, 64 in band. The promotion workflow now blocks the whole holdings/sector apply when any staged row is not valid, so partial data cannot reach runtime through the official job;
  - Motilal's official text layout now produces 91.67% manager and 100% risk coverage across 24 factsheet records locally. Its core fields clear the gate, but sector coverage remains 15% because the PDF publishes sector allocation separately from security rows; aggregate sector weights are not assigned to individual holdings. Motilal remains blocked until a separate reviewed sector-allocation staging path exists;
  - production acquisition runs `30343045400`, `30343047236`, and `30343049424` stored or reused the Motilal, Kotak, and supplied ABSL PDFs in R2. Exact deterministic parser run `30343289910` staged Motilal and Kotak. Retry `30344644933` proved the ABSL alias fix works, then safely left the supplied PDF in `needs_review` with `factsheet_content_report_month_mismatch:2026-05-01!=2026-06-01`;
  - no newly added AMC is `runtime_enabled`; each still requires June 2026 official discovery, R2 evidence, reviewed mapping, per-scope dry-run/apply, and live API/chat/canvas/Data & Trust acceptance.
  - latest completed verification: `536 passed, 6 skipped` backend tests. The previously completed frontend verification remains `58 passed` contracts plus TypeScript, ESLint, and the Next.js production build. The exact-source/combined-factsheet, ABSL retry-alias, and Motilal risk/manager commits are pushed; the promotion RPC repair migration is locally verified and pending approval/application.
- MF storage controls:
