...
- Comparison canvas delivery hardening:
  ...
  - current local pipeline hardening changes the 90% rollout gate to the true supported intersection (fresh history plus non-null alpha and beta divided by all supported mapped schemes), retries only Supabase HTTP 502/503/504/522/525 failures on repeat-safe operations, reconciles stale provider runs to `timed_out`, and scopes scheduled document automation to PPFAS, SBI, and Mirae. Nippon/UTI require exact approved document IDs; Axis/DSP/Motilal/HDFC are validation-only; ABSL/ICICI/Kotak remain frozen by GitHub issue #2. Backend verification passes `664` tests with `6` skipped; deployment is pending.
