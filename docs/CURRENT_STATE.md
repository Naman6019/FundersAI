...
- Twelve-AMC production readiness is implemented and production staging is active, but full runtime activation is not complete. See `MF_12_AMC_PRODUCTION_READINESS_RUNBOOK.md` for the execution procedure:
  ...
  - production parser diagnostics identified percentage-scale errors for Mirae, DSP, and Aditya Birla, plus HDFC page-text sector summaries being merged into coordinate-based holdings. The deterministic parser corrections are locally verified. Parsed records whose content month differs from their source document month are now routed to review without staging, batch promotion removes duplicate source versions while retaining distinct official portfolio files, and manual parser retries accept an exact report-month boundary. Approved staging-only June 2026 reparse run `30375656288` covers HDFC, Mirae, DSP, and Aditya Birla with deterministic extraction, LLM and runtime promotion disabled;
  ...
  - latest tracked backend verification is `573 passed, 6 skipped`; the broad `backend/tests` discovery command remains obstructed by a pre-existing inaccessible `backend/tests/tmp` directory, so verification explicitly uses the repository's tracked `test_*.py` modules. The previously completed frontend verification remains 58 contract tests plus TypeScript, ESLint, and the Next.js production build.
- MF storage controls:
