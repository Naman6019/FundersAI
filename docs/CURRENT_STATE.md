...
- Twelve-AMC production readiness is implemented and production staging is active, but full runtime activation is not complete. See `MF_12_AMC_PRODUCTION_READINESS_RUNBOOK.md` for the execution procedure:
  ...
  - Motilal apply `30454519921` succeeded, but the stricter runtime gate correctly rejects production readiness: risk 100%, TER/AUM 95.83%, benchmark 95.83%, manager 91.67%, holdings 85%, and sectors 90% pass while holdings ISIN coverage is 0%. Its official document API returns current records only when year/month filters are blank; local discovery now resolves the June `Scheme Portfolio Details` XLSX instead of reusing the factsheet PDF. Acquisition, staging-only parsing, reviewed portfolio promotion, and live ISIN verification remain pending;
