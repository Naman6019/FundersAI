...
- Twelve-AMC production readiness is implemented and production staging is active, but full runtime activation is not complete. See `MF_12_AMC_PRODUCTION_READINESS_RUNBOOK.md` for the execution procedure:
  ...
  - protected portfolio dry-runs `30376674873` and `30376677532` made no runtime writes. DSP was rejected because the deployed writer still treats any rejected staged row as a full-scope failure despite 3,679 promotable holdings rows. Aditya Birla hit PostgREST statement timeout `57014` while paging a large source document. The locally verified fix adds composite source-document indexes and a new service-role-only valid-row promotion RPC with independent 80% distinct-family mapping and validation gates; migration `20260728_add_mf_thresholded_portfolio_promotion_v2.sql` is not yet applied;
  ...
  - remaining production gates are: apply the pending thresholded-portfolio promotion migration, deploy and run the bounded parser correction for HDFC/DSP/Aditya Birla, repair Nippon and Kotak continuation/duplicate parsing, rerun independent-scope dry-runs, apply only passing scopes, verify runtime coverage and selected live APIs, verify the exact HDFC `118989` versus Nippon `118668` chat/canvas acceptance case, verify Data & Trust, and only then enable each new AMC for users;
  - latest tracked backend verification is `579 passed, 6 skipped`; the broad `backend/tests` discovery command remains obstructed by a pre-existing inaccessible `backend/tests/tmp` directory, so verification uses `--ignore=backend/tests/tmp`. The previously completed frontend verification remains 58 contract tests plus TypeScript, ESLint, and the Next.js production build.
- MF storage controls:
