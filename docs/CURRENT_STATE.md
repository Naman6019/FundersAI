...
- Twelve-AMC production readiness is implemented and production staging is active, but full runtime activation is not complete. See `MF_12_AMC_PRODUCTION_READINESS_RUNBOOK.md` for the execution procedure:
  ...
  - maturity dates and other incidental workbook dates were still being mistaken for report months in HDFC, DSP, Aditya Birla, and Mirae parser evidence. The follow-up parser fix now prefers the source document month, permits only explicit scheme-local `as on`/`as of` overrides, and rejects long regulatory disclosure footnotes as scheme titles. This fix is locally verified but has not yet been deployed or reparsed in production;
  ...
  - protected HDFC all-scope dry-run `30436455166` made no runtime writes and rejected the batch because 21 portfolio targets from 11 review-only XLSX sources failed their per-source 80% gates. The other 181 targets were promotable. The batch orchestrator now quarantines only those recognized portfolio-coverage rejections when the families that would actually be applied still pass the 80% AMC gate; source-integrity, month, storage, or unknown rejection reasons remain blocking. The exact production-backed local replay passes with zero unsafe rejections, 92.78% post-apply holdings-family coverage, and 91.11% post-apply sector-family coverage. This correction is locally verified and not yet deployed;
  - the parser-sync workflow now uses the promotion-eligible staging report as its strict current-month gate. The older live-table coverage script was removed from this pre-promotion workflow because it necessarily reports missing runtime values before promotion and could fail a successful staging run. This workflow correction is implemented locally and awaits verification and deployment;
  - remaining production gates are: deploy the strict staging and safe batch-quarantine corrections, rerun protected independent-scope dry-runs, apply only passing scopes, verify runtime coverage and selected live APIs, verify the exact HDFC `118989` versus Nippon `118668` chat/canvas acceptance case, verify Data & Trust, and only then enable each new AMC for users;
  - latest tracked backend verification is `585 passed, 6 skipped`; the HDFC pandas/null regression subset passes `43 passed, 4 skipped`, and the exact acquired XLSX also passes under the runner's pandas `3.0.5`. The broad `backend/tests` discovery command remains obstructed by a pre-existing inaccessible `backend/tests/tmp` directory, so verification uses `--ignore=backend/tests/tmp`. The previously completed frontend verification remains 58 contract tests plus TypeScript, ESLint, and the Next.js production build.
- MF storage controls:
