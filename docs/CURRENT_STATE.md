...
- Twelve-AMC production-readiness implementation is partially pushed, with a small local follow-up diff; it is not deployed or activated. See `MF_12_AMC_PRODUCTION_READINESS_RUNBOOK.md` for the current execution snapshot:
  ...
  - latest completed broad backend verification remains `540 passed, 6 skipped`; the current sector-staging change has `62 passed` focused tests and local parsing evidence. The previously completed frontend verification remains `58 passed` contracts plus TypeScript, ESLint, and the Next.js production build.
- MF storage controls:
