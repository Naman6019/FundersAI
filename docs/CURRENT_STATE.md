...
- Twelve-AMC production-readiness implementation is partially pushed, with a small local follow-up diff; it is not deployed or activated. See `MF_12_AMC_PRODUCTION_READINESS_RUNBOOK.md` for the current execution snapshot:
  ...
  - the reviewed Aditya Birla source is now the exact official `Empower Factsheet - June 2026` PDF, not the prior July-named checksum-conflict candidate. Ranged validation confirms a 10,837,692-byte PDF. Local parsing finds 96 records with 83.33% TER, 98.96% benchmark, and 94.79% manager coverage; AUM and risk remain below the production gate. The exact June 30 portfolio ZIP remains staged with 91.30% mapping, 100% holdings coverage, and 97.33% sector coverage;
  ...
  - the latest exact-June staging audit has seven full passes: HDFC, SBI, ICICI, PPFAS, Nippon, Mirae, and DSP. Axis, UTI, Kotak, Aditya Birla, and Motilal remain blocked pending corrected acquisition/parsing and a fresh staging report;
  - reviewed mapping coverage is HDFC 97.94% core / 98.89% portfolio, SBI 92.47% / 85.47%, ICICI 98.48% / 98.56%, PPFAS 100% / 100%, Nippon 98.72% / 100%, Mirae 98.90% / 97.75%, DSP 98.57% / 87.21%, Axis 96.34% / 90.70%, UTI 90.41% / 97.47%, and Kotak 95.73% / 0%. Core promotion dry-runs for the seven passing AMCs and their exact source documents succeeded; no apply promotion has run;
  - exact official June sources are now locally verified for Motilal, Kotak, and Aditya Birla. Motilal and Kotak combined-factsheet portfolio extraction is locally implemented without the slow generic PDF table scan. Motilal yields 20 portfolio records, 19 in the valid allocation band; Kotak yields 76 records, 64 in band. Rows outside the valid allocation band are staged for review and are explicitly rejected by promotion;
  - production acquisition runs `30343045400`, `30343047236`, and `30343049424` stored or reused the exact Motilal, Kotak, and ABSL June PDFs in R2. Exact deterministic parser run `30343289910` staged Motilal and Kotak; ABSL remained pending because the retry job filtered `aditya_birla` without resolving its production database code `ABSL`. Registry-based retry alias resolution and its regression test are locally verified but not yet pushed or rerun;
  ...
  - latest local verification: `534 passed, 6 skipped` backend tests, `58 passed` frontend contracts, TypeScript, ESLint, and the Next.js production build. The exact-source/combined-factsheet commit is pushed and acquired/staged as described above; the ABSL retry alias follow-up is not yet pushed or rerun.
- MF storage controls:
