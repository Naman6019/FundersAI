...
- Twelve-AMC production readiness is implemented and production staging is active, but full runtime activation is not complete. See `MF_12_AMC_PRODUCTION_READINESS_RUNBOOK.md` for the execution procedure:
  ...
  - the latest read-only June promotion-eligible report passes every 80% field and mapping gate for 11 of 12 AMCs. SBI, ICICI, Axis, PPFAS, Nippon, Motilal, Mirae, UTI, DSP, Kotak, and Aditya Birla pass. HDFC remains below the portfolio gates at 35.96% holdings and 39.74% sectors; its core fields pass;
  ...
  - HDFC's official monthly-portfolio page is now the authoritative portfolio source. A locally verified June inventory contains 107 official-host XLSX objects and records one page-listed Banking Financial Services object as unavailable rather than attempting an invalid download. Discovery merges this bounded reviewed inventory with live page results, prefers the XLSX files to PDF fallbacks, and keeps acquisition separate. The inventory has not yet been deployed or acquired;
  ...
  - remaining production gates are: deploy and acquire the reviewed HDFC June per-scheme portfolio XLSX inventory, stage and measure it, rerun current independent-scope dry-runs, apply only passing scopes, verify runtime coverage and selected live APIs, verify the exact HDFC `118989` versus Nippon `118668` chat/canvas acceptance case, verify Data & Trust, and only then enable each new AMC for users;
  - latest tracked backend verification is `585 passed, 6 skipped`; the HDFC discovery/inventory changes also pass 75 focused downloader, readiness, and parser tests. The broad `backend/tests` discovery command remains obstructed by a pre-existing inaccessible `backend/tests/tmp` directory, so verification uses `--ignore=backend/tests/tmp`. The previously completed frontend verification remains 58 contract tests plus TypeScript, ESLint, and the Next.js production build.
- MF storage controls:
