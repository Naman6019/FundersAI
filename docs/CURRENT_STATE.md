...
- Twelve-AMC production readiness is implemented and production staging is active, but full runtime activation is not complete. See `MF_12_AMC_PRODUCTION_READINESS_RUNBOOK.md` for the execution procedure:
  ...
  - production staging report `30363634114` confirms ten complete 80% passes: HDFC, SBI, ICICI, Axis, PPFAS, Nippon, Motilal, Mirae, DSP, and Kotak. Its workflow conclusion is red only because the later runtime coverage gate correctly detects scopes that have not yet been promoted;
  - exact current mapping coverage is HDFC 97.94% core / 98.89% portfolio, SBI 92.47% / 85.47%, ICICI 98.48% / 100%, Axis 96.34% / 90.70%, PPFAS 100% / 100%, Nippon 98.72% / 100%, Motilal 100% / 100%, Mirae 98.90% / 97.47%, DSP 98.57% / 87.21%, Kotak 95.73% / 96.05%, UTI 90.41% / 97.47%, and Aditya Birla 0% / 91.30%;
  - the ten passing AMCs also clear every individual AUM, TER, benchmark, manager, risk, holdings, and applicable-sector threshold. UTI remains below the core gate because its active July-published Fund Watch was stored as July; Aditya Birla has no June core candidates for the same publication-month/body-month reason;
  ...
  - neither report-month correction has been applied. Each still requires its exact protected apply phrase, followed by deterministic reparse and a fresh twelve-AMC staging report;
  ...
  - remaining production gates are: apply the two reviewed month corrections, reparse UTI and Aditya Birla, prove 12/12 staging coverage, run/review bounded per-AMC dry-runs, apply each passing scope, verify runtime coverage and selected live APIs, verify the exact HDFC `118989` versus Nippon `118668` chat/canvas acceptance case, verify Data & Trust, and only then enable each new AMC for users;
