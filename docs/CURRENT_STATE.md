...
- Twelve-AMC production readiness is implemented and production staging is active, but full runtime activation is not complete. See `MF_12_AMC_PRODUCTION_READINESS_RUNBOOK.md` for the execution procedure:
  ...
  - the production database has the extraction staging, hardened/atomic promotion, sector-allocation staging, compact staging-coverage RPC, and audited report-month reconciliation migrations applied. No new secret was added;
  - production staging report `30368307110` confirms all 12 AMCs pass the 80% distinct-family gate for AUM, TER, benchmark, manager, risk, holdings, and sectors. Its workflow conclusion is red only because the later legacy runtime checker correctly detects scopes that have not yet been promoted;
  - exact current mapping coverage is HDFC 97.94% core / 98.89% portfolio, SBI 92.47% / 85.47%, ICICI 98.48% / 100%, Axis 96.34% / 90.70%, PPFAS 100% / 100%, Nippon 98.72% / 100%, Motilal 100% / 100%, Mirae 98.90% / 97.47%, DSP 98.57% / 87.21%, Kotak 95.73% / 96.05%, UTI 89.87% / 97.47%, and Aditya Birla 94.68% / 91.30%;
  ...
  - single-source and bounded AMC/month promotion workflows keep acquisition separate from promotion, require the `production-data` reviewer, validate all selected targets before mutation, enforce a 150-source hard cap, and apply core, holdings, and sectors independently. The AMC batch runner has 34 focused passing tests and is locally verified; its first production dry-runs are still pending;
  - remaining production gates are: resolve or explicitly review the eight unmatched UTI/Aditya Birla names, run/review bounded per-AMC promotion dry-runs, apply each passing scope, verify runtime coverage and selected live APIs, verify the exact HDFC `118989` versus Nippon `118668` chat/canvas acceptance case, verify Data & Trust, and only then enable each new AMC for users;
  - latest completed broad backend verification remains `540 passed, 6 skipped`. The UTI detector/reconciliation suite has 63 focused passing tests; the bounded AMC promotion change has 34 focused passing tests. The previously completed frontend verification remains 58 contract tests plus TypeScript, ESLint, and the Next.js production build.
- MF storage controls:
