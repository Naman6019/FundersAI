...
- Twelve-AMC production-readiness implementation is partially pushed, with a small local follow-up diff; it is not deployed or activated. See `MF_12_AMC_PRODUCTION_READINESS_RUNBOOK.md` for the current execution snapshot:
  ...
  - `20260727_add_mf_extraction_staging_and_promotion.sql`, both RPC-hardening repairs, and `20260728_make_mf_factsheet_promotion_atomic.sql` are applied. The live four-argument factsheet promotion no longer delegates to the legacy RPC and preserves scalar provider evidence under `legacy_provider_payload`;
  - exact Kotak June core dry-run `30356511523` and apply `30356643200` succeeded. The writer returned 113 applied scheme operations across each candidate's eligible subset of risk, TER/AUM, benchmark, and manager. Live `/api/mf/148606` verification returned AUM `746.02`, TER `2.39`, benchmark `Nifty 100 ESG Index TRI`, risk `Very High`, manager `Mr. Mandar Pawar`, and the promotion timestamp;
  ...
  - the latest exact-June staging audit has eight full passes: HDFC, SBI, ICICI, PPFAS, Nippon, Mirae, DSP, and Kotak. Axis retry run `30351866335` subsequently completed successfully after its TER and risk parser fixes, but a fresh 12-AMC staging report is still required before counting Axis as a production gate pass. UTI and Aditya Birla still require audited content-confirmed report-month correction and reparse;
  - reviewed mapping coverage is HDFC 97.94% core / 98.89% portfolio, SBI 92.47% / 85.47%, ICICI 98.48% / 98.56%, PPFAS 100% / 100%, Nippon 98.72% / 100%, Mirae 98.90% / 97.75%, DSP 98.57% / 87.21%, Kotak 95.73% / 96.05%, Axis 96.34% / 90.70%, UTI 90.41% / 97.47%, Motilal 100% / 100%, and Aditya Birla 0% / 91.30%. Core promotion dry-runs for the first seven passing AMCs succeeded. Kotak's protected core apply run `30356643200` subsequently promoted 113 reviewed scheme candidates, and the live first-party MF API now returns its promoted AUM, TER, risk, benchmark, and manager values;
  ...
  - Motilal's official text layout produces 91.67% manager and 100% risk coverage across 24 factsheet records. The locally verified aggregate-sector parser now extracts 274 official rows across 18 of 20 parsed portfolio schemes; the two remaining schemes are arbitrage/debt and sector-inapplicable. `20260728_add_mf_sector_allocation_staging.sql` keeps these aggregate weights separate from holdings and adds atomic, independently validated sector promotion. The migration is locally verified and pending application before production reparse;
  - audited UTI/ABSL report-month reconciliation is repository-prepared but not deployed or run. `20260728_add_mf_report_month_reconciliation.sql` adds an append-only audit record and a service-role RPC; the protected workflow independently downloads the exact R2 object, verifies its SHA-256, detects the official body month, rejects any already-promoted document, and only then can clear that document's stale staging rows and queue `needs_reparse`. Each apply still requires exact approval;
  ...
  - latest completed broad backend verification remains `540 passed, 6 skipped`; the sector-staging change has `84 passed, 4 skipped` focused tests and local parsing evidence. The previously completed frontend verification remains `58 passed` contracts plus TypeScript, ESLint, and the Next.js production build.
- MF storage controls:
