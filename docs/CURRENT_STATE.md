...
- Twelve-AMC production-readiness implementation is partially pushed, with a small local follow-up diff; it is not deployed or activated. See `MF_12_AMC_PRODUCTION_READINESS_RUNBOOK.md` for the current execution snapshot:
  ...
  - Motilal uses its official bounded AEM document-search API. A local 2026-06 acceptance pass correctly rejected the newest May factsheet and April month-end portfolio as stale and excluded fortnightly/performance files instead of treating the API publication bucket as the reporting month;
  ...
  - the official Aditya Birla Sun Life July 2026 factsheet is a validated June-content candidate but is not yet acquired because the acquisition workflow did not load the reviewed manifest. The exact June 30 portfolio ZIP is parsed into 105 schemes and 5,928 staged holdings; portfolio mapping is 91.30%, holdings coverage is 100%, and sector coverage is 97.33%;
  ...
  - HDFC's acquired combined factsheet/portfolio still reflects the older production parser result. The official June Index Solutions factsheet is now in the reviewed manifest; local extraction across the main and Index Solutions files covers AUM 100%, TER 81.44%, benchmark 100%, manager 87.63%, and risk 100% across 97 unique scheme keys. Production acquisition and staging must be refreshed before HDFC passes. Axis still has no exact-June raw row because the last acquisition run did not load the reviewed manifest fallback;
  ...
  - the 2026-07-28 exact-June staging audit has five full passes: SBI, ICICI, PPFAS, Mirae, and DSP. HDFC, Nippon, and UTI have current staged core and portfolio evidence but fail one or more field gates; Axis and Motilal have no June staged rows; Kotak is factsheet-only; Aditya Birla is portfolio-only with 91.30% reviewed mapping and no factsheet staging;
  - reviewed mapping coverage is SBI 92.47% core / 85.47% portfolio, ICICI 98.48% / 98.56%, PPFAS 100% / 100%, Mirae 98.90% / 97.75%, DSP 98.57% / 87.21%, HDFC 97.67% / 100%, Nippon 98.72% / 100%, UTI 90.28% / 97.47%, and Kotak 95.73% / 0%. No promotion RPC was called;
  - current acquisition gaps remain explicit: Axis requires the reviewed manifest to be loaded by the acquisition workflow; Aditya Birla needs the same fix for its June-content factsheet; Kotak's official portfolio listing remains WAF-protected; Motilal's official API still exposes May factsheet and April portfolio data. These AMCs are not production-ready;
  ...
  - latest local verification: `529 passed, 6 skipped` backend tests, including `115 passed` across the focused factsheet parsing, AMC discovery, production-readiness, parsing-scope, and staging-coverage set. The prior frontend baseline remains `58 passed` contracts plus TypeScript, ESLint, and the Next.js production build; all 20 workflow files parse as YAML. The follow-up diff is not yet pushed, deployed, reacquired, or restaged.
- MF storage controls:
